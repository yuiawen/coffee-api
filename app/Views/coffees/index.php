...
<?= $this->section('content') ?>
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-3xl font-bold text-slate-800">Manajemen Produk Kopi</h1>
            <p class="text-slate-500 mt-1">Kelola semua produk kopi Anda di sini.</p>
        </div>
        <a href="/coffees/new" class="flex items-center gap-2 py-2 px-4 border border-transparent rounded-lg shadow-sm text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700">
    ...
    <div class="bg-white p-6 rounded-xl shadow-lg">
        <div class="flex justify-between items-center mb-4">
            ...
            <form action="/coffees" method="get" class="relative">
                    ...
                    <?php if (!empty($coffees)): ?>
                        <?php foreach ($coffees as $coffee): ?>
                            <tr class="hover:bg-slate-50">
                                <td class="px-6 py-4">
                                    <img src="<?= base_url('uploads/' . $coffee['image']) ?>" alt="<?= esc($coffee['name']) ?>" class="h-12 w-12 rounded-lg object-cover">
                                </td>
                                <td class="px-6 py-4">
                                    <div class="text-sm font-semibold text-slate-800"><?= esc($coffee['name']) ?></div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="text-sm text-slate-800">Rp <?= number_format($coffee['price'], 0, ',', '.') ?></div>
                                </td>
                                <td class="px-6 py-4 text-sm font-medium">
                                    <div class="flex items-center gap-4">
                                        <a href="/coffees/<?= $coffee['id'] ?>/edit" class="text-blue-600 hover:text-blue-800 flex items-center gap-1">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor"><path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z" /><path fill-rule="evenodd" d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" clip-rule="evenodd" /></svg>
                                            Edit
                                        </a>
                                        <form action="/coffees/<?= $coffee['id'] ?>" method="post" onsubmit="return confirm('Apakah Anda yakin?');">
        ...
        <div class="mt-6">
            <?= $pager->links('coffees', 'default_full') ?>
        </div>
    </div>
<?= $this->endSection() ?>